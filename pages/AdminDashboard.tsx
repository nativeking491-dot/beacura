...
import { Users, TrendingUp, Award, Activity, Loader2, UserCheck, Calendar, Search, Filter, RefreshCw, Shield, X, BarChart3, Edit, Trash2, Save, AlertTriangle } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
    ...
    const fetchStats = async () => {
        ...
        try {
            ...
            if (users) {
                ...
                const recoveringUsers = users.filter(u => u.role === 'RECOVERING_USER').length;
                const mentors = users.filter(u => u.role === 'RECOVERED_MENTOR').length;
                ...
                const averageStreak = regularUsers.length > 0
                    ? Math.round(regularUsers.reduce((sum, u) => sum + (u.streak || 0), 0) / regularUsers.length)
                    : 0;

                const totalPoints = users.reduce((sum, u) => sum + (u.points || 0), 0);
                const activeToday = users.filter(u => u.role !== 'ADMIN' && (u.streak || 0) > 0).length;
    ...
    const StatCard = ({ icon: Icon, label, value, color, bgColor, subtitle, statId }: any) => (
        <div
        ...
        switch (statType) {
            ...
            case 'recovering':
                return {
                    ...
                    data: [
                        { name: 'Recovering Users', value: stats.recoveringUsers, fill: '#14b8a6' },
            ...
            case 'engagement':
                return {
                    ...
                    data: [
                        ...
                        { name: 'Total Points/100', value: Math.round(stats.totalPoints / 100) }
                    ],
                    type: 'bar'
                };
            default:
    ...
    const StatModal = ({ statId, onClose }: { statId: string, onClose: () => void }) => {
            ...
            switch (statId) {
                case 'total':
                    return {
                        ...
                        metrics: [
                            { label: 'Total Registered', value: stats.totalUsers, icon: Users },
                            { label: 'This Week', value: stats.newUsersThisWeek, icon: TrendingUp },
                            { label: 'This Month', value: stats.newUsersThisMonth, icon: Calendar },
                            { label: 'Growth Rate', value: `${stats.totalUsers > 0 ? Math.round((stats.newUsersThisWeek / stats.totalUsers) * 100) : 0}%`, icon: Activity }
                        ]
                    };
                case 'recovering':
                    return {
                        title: 'User Role Distribution',
                        users: allUsers.filter(u => u.role === 'RECOVERING_USER').slice(0, 10),
                        metrics: [
                            { label: 'Recovering Users', value: stats.recoveringUsers, icon: UserCheck },
                            { label: 'Mentors', value: stats.mentors, icon: Award },
                            { label: 'Admins', value: stats.admins, icon: Shield },
                            { label: 'Avg Streak (Recovery)', value: Math.round(allUsers.filter(u => u.role === 'RECOVERING_USER').reduce((sum, u) => sum + u.streak, 0) / Math.max(stats.recoveringUsers, 1)), icon: Activity }
                        ]
                    };
                case 'growth':
                    return {
                        ...
                        metrics: [
                            { label: 'New This Week', value: stats.newUsersThisWeek, icon: Calendar },
                            { label: 'New This Month', value: stats.newUsersThisMonth, icon: Calendar },
                            { label: 'Active Today', value: stats.activeToday, icon: Activity },
                            { label: 'Activation Rate', value: `${stats.totalUsers > 0 ? Math.round((stats.activeToday / stats.totalUsers) * 100) : 0}%`, icon: TrendingUp }
                        ]
                    };
                case 'engagement':
                    return {
                        title: 'User Engagement Details',
                        users: allUsers.filter(u => u.streak > 0).sort((a, b) => b.streak - a.streak).slice(0, 10),
                        metrics: [
                            { label: 'Average Streak', value: stats.averageStreak, icon: Activity },
                            { label: 'Total Points', value: stats.totalPoints.toLocaleString(), icon: Award },
                            { label: 'Active Users', value: stats.activeToday, icon: UserCheck },
                            { label: 'Engagement Rate', value: `${stats.totalUsers > 0 ? Math.round((stats.activeToday / stats.totalUsers) * 100) : 0}%`, icon: TrendingUp }
                        ]
                    };
                default:
                    return null;
                ...
                <div className="bg-white rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="p-8 space-y-8">
                        ...
                        <div className="bg-slate-50 p-6 rounded-xl">
                            <h3 className="text-lg font-bold text-slate-800 mb-4">Visual Analytics</h3>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {detailedData?.metrics.map((metric, index) => (
                                    <div key={index} className="bg-gradient-to-br from-purple-50 to-indigo-50 p-5 rounded-xl border border-purple-200">
                        ...
                        {detailedData && detailedData.users.length > 0 && (
                            <div>
                                <h3 className="text-lg font-bold text-slate-800 mb-4">
                                    {statId === 'total' ? 'Recent Users' :
                                        statId === 'recovering' ? 'Recovering Users' :
                                            statId === 'growth' ? 'New Users This Week' :
                                                'Top Engaged Users'}
                                </h3>
                                    ...
                                    <table className="w-full">
                                        <thead className="bg-gradient-to-r from-purple-100 to-indigo-100">
                                            <tr>
                                                ...
                                                <th className="text-right py-3 px-4 font-semibold text-slate-700">Streak</th>
                                                <th className="text-right py-3 px-4 font-semibold text-slate-700">Points</th>
                                                <th className="text-left py-3 px-4 font-semibold text-slate-700">Joined</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {detailedData.users.map((user, index) => (
                                                <tr key={user.id} className={`border-b border-slate-100 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
                                                    <td className="py-3 px-4 font-medium text-slate-900">{user.name}</td>
                                                    <td className="py-3 px-4 text-slate-600 text-sm font-mono">{user.email}</td>
                                                    <td className="py-3 px-4">
                                                        <span className={`px-2 py-1 rounded-full text-xs font-bold ${user.role === 'ADMIN'
                                                            ...
                                                            {user.role === 'ADMIN' ? '👑' : user.role === 'RECOVERED_MENTOR' ? '🏆' : '💚'}
                                                        </span>
                                                    </td>
                                                    <td className="py-3 px-4 text-right font-bold text-teal-600">{user.streak} 🔥</td>
                                                    <td className="py-3 px-4 text-right font-bold text-indigo-600">{user.points.toLocaleString()}</td>
                                ...
                                {detailedData.users.length >= 10 && (
                                    <p className="text-sm text-slate-500 mt-3 text-center">Showing top 10 users</p>
                                )}
                            </div>
                        )}
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                ...
                                <div className="text-center">
                                    ...
                                    <p className="text-3xl font-bold">
                                        {stats.totalUsers > 0 ? Math.round((stats.newUsersThisWeek / stats.totalUsers) * 100) : 0}%
                                    </p>
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setEditingUser(null)}>
                <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full" onClick={(e) => e.stopPropagation()}>
                    <div className="bg-gradient-to-r from-purple-600 to-indigo-600 p-6 text-white rounded-t-2xl flex justify-between items-center">
                        ...
                        <button onClick={() => setEditingUser(null)} className="p-2 hover:bg-white hover:bg-opacity-20 rounded-lg transition-colors">
                            <X size={24} />
                        </button>
                    </div>
                    ...
                    <div className="p-8 space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-semibold text-slate-700 mb-2">Streak (days)</label>
                                <input
                                    type="number"
    ...
    const DeleteConfirmModal = () => {
        ...
        return (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setDeletingUser(null)}>
                <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
                    <div className="bg-gradient-to-r from-rose-600 to-red-600 p-6 text-white rounded-t-2xl flex justify-between items-center">
                        ...
                        <button onClick={() => setDeletingUser(null)} className="p-2 hover:bg-white hover:bg-opacity-20 rounded-lg transition-colors">
                            <X size={24} />
                        </button>
                    </div>

                    <div className="p-8">
                        <p className="text-slate-700 text-lg mb-2">Are you sure you want to delete this user?</p>
                        <div className="bg-rose-50 border-l-4 border-rose-500 p-4 rounded-r-lg mb-6">
                            ...
                            <p className="text-sm text-slate-600 font-mono">{deletingUser.email}</p>
                        </div>
                        <p className="text-sm text-slate-500 mb-6">This action cannot be undone. All user data will be permanently removed.</p>
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center min-h-[60vh]">
                <div className="text-center">
                    <Loader2 className="animate-spin text-purple-600 mx-auto mb-4" size={56} />
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            {selectedStat && <StatModal statId={selectedStat} onClose={() => setSelectedStat(null)} />}
            <EditUserModal />
            <DeleteConfirmModal />
            ...
            <div className="bg-gradient-to-r from-purple-600 to-indigo-600 -m-8 mb-8 p-8 text-white rounded-b-3xl shadow-xl">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-purple-100 text-lg">Complete platform overview and user management</p>
                    </div>
                    <button
                        onClick={fetchStats}
            ...
            <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-lg">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 space-y-4 md:space-y-0">
                    <h2 className="text-2xl font-bold text-slate-900">All Users ({filteredUsers.length})</h2>
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-200">
                    <table className="w-full">
                        <thead className="bg-gradient-to-r from-purple-50 to-indigo-50">
                            <tr>
                                <th className="text-left py-4 px-6 font-bold text-slate-700">#</th>
                                <th className="text-left py-4 px-6 font-bold text-slate-700">Name</th>
                                ...
                                <th className="text-right py-4 px-6 font-bold text-slate-700">Streak</th>
                                <th className="text-right py-4 px-6 font-bold text-slate-700">Points</th>
                                <th className="text-left py-4 px-6 font-bold text-slate-700">Joined</th>
                                <th className="text-center py-4 px-6 font-bold text-slate-700">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filteredUsers.map((user, index) => (
                                <tr key={user.id} className="border-b border-slate-100 hover:bg-purple-50 transition-colors">
                                    <td className="py-4 px-6 text-slate-500 font-medium">{index + 1}</td>
                                    <td className="py-4 px-6">
                                        <span className="font-semibold text-slate-900">{user.name}</span>
                                    </td>
                                    <td className="py-4 px-6 text-slate-600 font-mono text-sm">{user.email}</td>
                                    <td className="py-4 px-6">
                                        <span className={`px-3 py-1.5 rounded-full text-xs font-bold ${user.role === 'ADMIN'
                                            ? 'bg-rose-100 text-rose-700 border border-rose-200'
                                            : user.role === 'RECOVERED_MENTOR'
                                                ? 'bg-amber-100 text-amber-700 border border-amber-200'
                                                : 'bg-teal-100 text-teal-700 border border-teal-200'
                                            }`}>
                                            {user.role === 'ADMIN' ? '👑 Admin' : user.role === 'RECOVERED_MENTOR' ? '🏆 Mentor' : '💚 User'}
                                        </span>
                                    </td>
                                    <td className="py-4 px-6 text-right">
                                        <span className="font-bold text-teal-600 text-lg">{user.streak} 🔥</span>
                                    </td>
                                    <td className="py-4 px-6 text-right">
                                        <span className="font-bold text-indigo-600 text-lg">{user.points.toLocaleString()}</span>
                                    </td>
