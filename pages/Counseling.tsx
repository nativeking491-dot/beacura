...
import {
  Star, MessageCircle, Video, Calendar,
  ShieldCheck, X, Check, Clock, Users,
  Send, Shield, Info, ArrowLeft, Phone, Smartphone,
  Loader2
} from 'lucide-react';
...
const Counseling: React.FC = () => {
  ...
  const [chatMessages, setChatMessages] = useState<Record<string, MentorMessage[]>>({});
  ...
  const sendMessage = async () => {
    ...
    const mentorInstruction = `You are ${activeChat.name}, a peer mentor who has been clean from drugs for ${activeChat.experience}. 
    Your specialties include ${activeChat.specialty.join(', ')}. 
    Speak as a human who has gone through the same struggles. Be empathetic, non-judgmental, and practical. 
    Use the user's input to provide specific peer support based on your shared experience. 
    Keep responses concise and supportive. If the user is in crisis, recommend professional emergency services.`;

    const aiResponse = await generateChatResponse(currentInput, mentorInstruction);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <header>
        ...
        <p className="text-slate-500">Professional peer support and community circles.</p>
      </header>
      ...
      <div className="bg-slate-900 text-white p-6 rounded-3xl flex items-center justify-between shadow-xl shadow-slate-200">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">Your real identity is only shared if you choose to.</p>
          </div>
        </div>
        <button className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl text-sm font-bold transition-all border border-white/10">
          Settings
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {MOCK_MENTORS.map((mentor) => (
          <div key={mentor.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-xl transition-all group">
            <div className="p-6">
              <div className="flex items-center space-x-4 mb-5">
                <div className="relative">
                  <img src={mentor.avatar} alt={mentor.name} className="w-16 h-16 rounded-2xl border-2 border-slate-50 object-cover shadow-sm group-hover:scale-105 transition-transform" />
                ...
                <div>
                  ...
                  <div className="flex items-center text-amber-500">
                    ...
                    <span className="text-slate-400 font-normal ml-2">{mentor.experience}</span>
                  </div>
                </div>
              </div>

              <div className="space-y-2 mb-6">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Focus Areas</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="flex items-center justify-center space-x-2 bg-slate-50 text-slate-700 py-3 rounded-2xl text-sm font-bold hover:bg-teal-50 hover:text-teal-700 transition-all border border-slate-100"
                >
                ...
                <button
                  ...
                  className="flex items-center justify-center space-x-2 bg-teal-600 text-white py-3 rounded-2xl text-sm font-bold hover:bg-teal-700 transition-all shadow-lg shadow-teal-100"
                >
                  <Video size={18} />
            ...
            <div className="bg-slate-50/50 px-6 py-4 border-t border-slate-100 flex items-center justify-between">
              ...
              <span className="text-[10px] font-black text-teal-600">BOOK NOW</span>
            </div>
          </div>
        ))}
        ...
        <div className="border-2 border-dashed border-slate-200 rounded-[2rem] flex flex-col items-center justify-center p-8 text-center bg-slate-50/30 group hover:bg-white transition-colors">
          ...
          <p className="text-slate-500 text-xs mb-4 max-w-[200px] mx-auto">Join others in a moderated group healing session.</p>
          <button className="bg-white border border-slate-200 px-6 py-2.5 rounded-xl text-xs font-bold hover:bg-slate-50 transition-colors shadow-sm">
            Browse Circles
          </button>
        </div>
      </div>
          ...
          <div className="bg-white w-full max-w-lg md:rounded-[2.5rem] shadow-2xl flex flex-col h-[90vh] md:h-[600px] animate-in slide-in-from-bottom-10 duration-500 overflow-hidden">
            ...
            <div className="p-5 border-b flex justify-between items-center bg-teal-600 text-white">
              <div className="flex items-center space-x-3">
                <img src={activeChat.avatar} className="w-10 h-10 rounded-xl object-cover border-2 border-white/20" />
                <div>
                  <h3 className="font-bold leading-none">{activeChat.name}</h3>
                  <span className="text-[10px] text-teal-100">Peer Mentor • {isTyping ? 'Typing...' : 'Online'}</span>
                </div>
              </div>
              <button onClick={closeModals} className="p-2 hover:bg-white/10 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>

            {/* Chat Messages */}
            <div ref={chatScrollRef} className="flex-1 overflow-y-auto p-5 space-y-4 bg-slate-50">
              {chatMessages[activeChat.id]?.map((msg) => (
                <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-4 rounded-2xl text-sm ${msg.sender === 'user'
                    ...
                    {msg.text}
                    <p className={`text-[10px] mt-1 ${msg.sender === 'user' ? 'text-teal-100' : 'text-slate-400'}`}>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center space-x-2">
                    ...
                    <span className="text-slate-400 text-xs font-medium">{activeChat.name} is typing...</span>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            {bookingStep === 'details' && (
              <div className="p-8">
                <div className="flex justify-between items-start mb-8">
                  <div className="flex items-center space-x-4">
                    <img src={selectedMentor.avatar} className="w-14 h-14 rounded-2xl object-cover shadow-sm" />
                    <div>
                      <h2 className="text-xl font-bold text-slate-900">Schedule Video Call</h2>
                      <p className="text-sm text-slate-500">with {selectedMentor.name}</p>
                    </div>
                  </div>
                  <button onClick={closeModals} className="p-2 hover:bg-slate-100 rounded-full transition-colors">
                    ...
                    <div className="flex space-x-3 overflow-x-auto pb-2 scrollbar-hide">
                      {dates.map((d, i) => (
                        ...
                        >
                          ...
                          <span className="text-[10px]">{d.date.split(' ')[0]}</span>
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            {bookingStep === 'payment' && (
              <div className="p-8 space-y-8 animate-in slide-in-from-right-10 duration-500">
                <div className="flex items-center space-x-4">
                  <button onClick={() => setBookingStep('details')} className="p-2 hover:bg-slate-100 rounded-full">
                    <ArrowLeft size={20} className="text-slate-600" />
                  </button>
                  <h2 className="text-xl font-bold text-slate-900">Payment via PhonePe</h2>
                </div>
                ...
                <div className="bg-indigo-50 p-6 rounded-3xl border border-indigo-100 text-center relative overflow-hidden">
                  ...
                  <p className="text-indigo-600 text-xs font-bold uppercase tracking-widest mb-1 relative z-10">Total Amount</p>
                  <p className="text-4xl font-black text-indigo-900 relative z-10">₹79</p>
                  <p className="text-[10px] text-indigo-500 mt-2 font-medium italic relative z-10">Instant verification via PhonePe/UPI</p>
                </div>

                <div className="space-y-4">
                  <div className="relative group">
                    <Smartphone className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-500 group-focus-within:text-indigo-600 transition-colors" size={18} />
                    <input
                      type="text"
                      value={upiId}
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl flex items-center space-x-3 border border-slate-100">
                    ...
                    <span className="text-xs text-slate-600 font-medium">Link your PhonePe account for faster checkout</span>
                  </div>
                </div>
              ...
              <div className="p-12 text-center space-y-6">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm">
                    Your call with <span className="text-slate-900 font-bold">{selectedMentor?.name}</span> is confirmed for
                    <span className="text-teal-600 font-bold"> {dates[selectedDate].date} at {selectedTime}</span>.
                  </p>
                </div>
                <div className="bg-slate-50 p-5 rounded-3xl border border-slate-100 text-left">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-700 mb-1">Meeting Link Sent</p>
                      <p className="text-[11px] text-slate-500 leading-relaxed">Check your email for the meeting link. The session will also appear on your dashboard.</p>
                    </div>
                  </div>
                </div>
                <button
                  onClick={closeModals}
                  className="w-full bg-teal-600 text-white py-4 rounded-2xl font-bold hover:bg-teal-700 transition-all shadow-xl shadow-teal-100"
                >
                  Back to Dashboard
                </button>
              </div>
